...
class LabelSmoothing(nn.Module):
    ...
    def __init__(self, size=0, padding_idx=0, smoothing=0.0):
        ...
        """ nn.KLDivLoss计算KL散度的损失函数，要将模型输出的原始预测值要先进行softmax，然后进行log运算
        （torch.nn.functional.log_softmax可以直接实现），得到结果作为input输入到KLDivLoss中。target是二维的，形状与input一样
        
        torch.nn.CrossEntropyLoss()交叉熵计算，输入的预测值不需要进行softmax，不需要进行log运算！直接用原始的预测输出，标签用整数序列。
        """
...
def clip_gradient(optimizer, grad_clip): # 梯度截断Clip, 将梯度约束在某一个区间之内. 避免梯度爆炸
    for group in optimizer.param_groups:
