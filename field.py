...
class ImageDetectionsField(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None, max_detections=100,
                 max_detections_gl=80, sort_by_prob=False, load_in_tmp=True,
                 scene_detections_path='/home/mlw/project/meshed-memory-transformer-master/feature/f30ktalk_att',
                 object_detections_path='/home/mlw/project/meshed-memory-transformer-master/feature/f30kbu_att'):
        self.max_detections = max_detections
        self.detections_path = detections_path
        self.scene_detections_path = scene_detections_path
        self.object_detections_path = object_detections_path
        self.sort_by_prob = sort_by_prob
        self.max_detections_gl = max_detections_gl
    ...
    #
    #     delta = self.max_detections - precomp_data.shape[0]  # 50-22=28
    #     if delta > 0:
    ...
    def preprocess(self, x, avoid_precomp=False):
        ...
        fl = np.load(self.object_detections_path + '/%s.npy' % image_name)
        lo_precomp_data = fl
        fg = np.load(self.scene_detections_path + '/%s.npz' % image_name)
        gl_precomp_data = fg['feat'][49:245, :]
        return lo_precomp_data.astype(np.float32), gl_precomp_data.astype(np.float32)
